...
public class PdsFile {
	...
	public static void save(
		...
		for (MultipartFile uploadfile : uploadfiles) {
			...
			String  fileExt  = 
					( orgName.lastIndexOf(".") < 0 ) // 못찾으면 -1
					? " "
					: orgName.substring( orgName.lastIndexOf(".") ); // .포함해서
			
			// 날짜 폴더 생성
			String  folderPath  = makeFolder( uploadPath );
			
			// 파일 중복 방지 : 같은 폴더에 같은 파일명을 저장하면 마지막 저장된 파일로 변경됨
			// 중복되지 않는 고유한 문자열 생성
			String  uuid        = UUID.randomUUID().toString();
			
			// 저장할 sfilename 생성
			// saveName : 실제 저장될 서버의 경로 + uuid + 파일명
	...
	private static String makeFolder(String uploadPath) {
		...
		// mkdir()  : 상위폴더가 없으면 폴더생성 실패
		// mkdirs() : 상위폴더가 없으면 폴더 전체를 생성
